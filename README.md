...
### ❌ "ValueError: ... not divisible by weight quantization block_n = 128"

Эта ошибка возникает при попытке запустить некоторые MoE-модели (например, Qwen3-VL-30B-FP8) с квантованием FP8. 
- **Причина:** Архитектура модели (размер экспертов) не кратна 128, что требуется для текущей реализации vLLM FP8.
- **Решение:** 
  1. Используйте модель в формате BF16 (например, Qwen3-VL-8B).
  2. Используйте квантование AWQ вместо FP8 (например, Qwen2.5-VL-72B-AWQ).
  3. Обновите vLLM до версии, где эта проблема исправлена (следите за релизами vLLM).
